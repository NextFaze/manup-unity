...
namespace NextFaze
{
    ...
    public class ManUp : MonoBehaviour
    {
        ...
        static readonly string platformOSXKey = "macos";
...
#region UI
        ...
        void SetupUI()
        {
            ...
            try
            {
                ...
                this.CurrentVersion = new System.Version(Application.version);

#if UNITY_EDITOR
                if (!string.IsNullOrEmpty(this.versionOverride))
                {
                    this.CurrentVersion = new System.Version(this.versionOverride);
                }
#endif
...
#region ManUp Config parsing
        ...
        IEnumerator DownloadConfig()
        {
            ...
            UnityWebRequest configFile = new UnityWebRequest(this.ConfigURL);;
            configFile.downloadHandler = new DownloadHandlerBuffer();
            yield return configFile.SendWebRequest();
    
            if((configFile.result == UnityWebRequest.Result.ConnectionError) || (configFile.result == UnityWebRequest.Result.ProtocolError)) {
                LogToFile("Download error: " + configFile.error);
                
                LoadLocalConfig();
            }
            else {
                string jsonString = configFile.downloadHandler.text;
        ...
        internal void ParseConfigJSON(string configJSON)
        {
            ...
            rootNode = JSON.Parse(configJSON);
            ...
            this.LatestVersion = new System.Version(platformNode[appVersionLatestKey]);
            this.MinimumVersion = new System.Version(platformNode[appVersionMinKey]);
        ...
        public JSONNode SettingForKey(string key) {
            return rootNode[key] ?? rootNode[platformKey][key];
        }
